...
function setTrending(name) {
    ...
    const styledName = applyRandomFont(searchInput.value); // Use the name for styling
...
function randomLikesDislikes() {
    ...
    return `❤️ ${likes} Likes | 👎 ${dislikes} Dislikes`;
}
...
function updateNicknames() {
    ...
    if (baseName) {
        const nicknames = generateStylishNicknames(baseName, 100);
...
function loadMoreNicknames() {
    const baseName = decodeHtmlEntities(document.getElementById('searchInput').value.trim());
    if (baseName) {
        const nicknames = generateStylishNicknames(baseName, 100);
        displayNicknames(nicknames);
    } else {
